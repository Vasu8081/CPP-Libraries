...
public:
    void deleten(T data){
        ...
        auto temp = root->data;
        if(root->leftChild==NULL && root->rightChild == NULL && temp == data){
            free(root);
            root = NULL;
        }
        auto node = searchnode(data);
        if(nextLeaf->rightChild!=NULL){
            node->data = nextLeaf->rightChild->data;
            auto tempN = nextLeaf->rightChild;
            nextLeaf->rightChild = NULL;
            free(tempN);
            heapify_parent(node);
            ...
            node->data = nextLeaf->leftChild->data;
            auto tempN = nextLeaf->leftChild;
            nextLeaf->leftChild = NULL;
            free(tempN);
            heapify_parent(node);
...
public:
    T get_min(Node* node){
        ...
        T maxLeft=std::numeric_limits<T>::max();
        T maxRight = maxLeft;
        if(node->leftChild)
            maxLeft = get_min(node->leftChild);
        if(node->rightChild)
            maxRight = get_min(node->rightChild);
        if(maxLeft<maxRight){
            return maxLeft;
        }
        else{
