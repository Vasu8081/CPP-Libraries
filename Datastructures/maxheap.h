...
P create_max_heap(P root, T data, K type){
...
template <typename P, typename T, typename K>
void insert_max_heap(P root, K data, T type){
...
template <typename P, typename K>
K get_min(P node, K data){
    if(node->leftChild == NULL && node->rightChild==NULL){
        return node->data;
    }
    K maxLeft=std::numeric_limits<K>::max();
    K maxRight = maxLeft;
    if(node->leftChild)
    maxLeft = get_min(node->leftChild, data);
    if(node->rightChild)
    maxRight = get_min(node->rightChild, data);
    if(maxLeft<maxRight){
        return maxLeft;
    }
    else{
